...
        try {
            // DB에서 모든 사용자 정보 필드를 가져오도록 수정
            const userResult = await pool.query('SELECT id, login_id, email, password_hash, username, profile_image_url, phone_number, address, is_social_user, is_admin FROM users WHERE login_id = $1', [login_id]);
